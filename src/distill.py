"""Train a pretrained width-scaled MobileNetV2 student with KD, then QAT + KD."""
...
def kd_epoch(student, teacher, loader, device, optimizer, alpha, temperature,
             label_smoothing=0.0, max_batches=None, freeze_bn=False):
    ...
    with context:
        for index, (images, labels) in enumerate(loader):
            ...
            loss = ((1 - alpha) * F.cross_entropy(logits, labels, label_smoothing=label_smoothing) + alpha * temperature ** 2 *
...
def args_parser():
    ...
    parser.add_argument("--temperature", type=float, default=2); parser.add_argument("--alpha", type=float, default=.25)
    parser.add_argument("--label-smoothing", type=float, default=.1)
    parser.add_argument("--freeze-backbone-epochs", type=int, default=2)
    parser.add_argument("--student-init", choices=("teacher-slice", "random"), default="teacher-slice",
                        help="Initialize a narrow student from overlapping tensors in the trained teacher.")
...
@torch.no_grad()
def initialize_from_teacher(student: nn.Module, teacher: nn.Module) -> dict[str, int]:
    """Copy the overlapping prefix of every compatible teacher tensor.

    torchvision only publishes MobileNetV2 weights at width 1.0.  Slicing the
    fine-tuned width-1 teacher is a deterministic pretrained initialization for
    narrower students and also transfers its CIFAR-10 classifier where shapes
    permit.  All target dimensions must fit inside the corresponding source.
    """
    teacher_state = teacher.state_dict(); student_state = student.state_dict()
    copied_tensors = copied_values = total_values = 0
    for name, target in student_state.items():
        total_values += target.numel()
        source = teacher_state.get(name)
        if source is None or source.ndim != target.ndim or any(t > s for t, s in zip(target.shape, source.shape)):
            continue
        if target.ndim == 0:
            target.copy_(source)
        else:
            target.copy_(source[tuple(slice(0, size) for size in target.shape)])
        copied_tensors += 1; copied_values += target.numel()
    student.load_state_dict(student_state)
    return {"copied_tensors": copied_tensors, "copied_values": copied_values,
            "student_values": total_values}


def main():
    args = args_parser().parse_args()
    if args.stage == "qat" and not args.student_checkpoint: raise ValueError("--student-checkpoint is required for QAT")
    if (args.width_mult <= 0 or args.temperature <= 0 or not 0 <= args.alpha <= 1 or
            not 0 <= args.label_smoothing < 1 or args.freeze_backbone_epochs < 0):
        raise ValueError("invalid width/KD settings")
    args.epochs = args.epochs or (60 if args.stage == "fp32" else 12)
    args.learning_rate = args.learning_rate or (.01 if args.stage == "fp32" else 3e-4)
    ...
    initialization = None
    if args.stage == "fp32":
        student = build_model(student_config, load_pretrained=False).to(device)
        if args.student_init == "teacher-slice": initialization = initialize_from_teacher(student, teacher)
    else:
    ...
    augmentation = None if args.stage == "qat" or args.no_strong_augmentation else {
        "randaugment": True, "randaugment_num_ops": 2,
        "randaugment_magnitude": 7, "random_erasing_probability": .10}
    train_loader, val_loader, _ = build_cifar10_loaders(
        args.data_dir, args.batch_size, args.num_workers, device.type == "cuda",
        args.seed, validation_size, augmentation)
    ...
    weight_decay = 1e-4 if args.stage == "fp32" else 4e-5
    optimizer = torch.optim.SGD([{"params": weights, "lr": args.learning_rate, "weight_decay": weight_decay}, {"params": scales, "lr": args.learning_rate, "weight_decay": 0}], momentum=.9, nesterov=True)
    ...
    realized = None
    if args.stage == "fp32" and args.freeze_backbone_epochs:
        freeze_backbone(student, True)
    for epoch in range(1, args.epochs + 1):
        if args.stage == "fp32" and args.freeze_backbone_epochs and epoch == args.freeze_backbone_epochs + 1:
            freeze_backbone(student, False)
        at_target = True
        if args.stage == "qat":
        ...
        started = perf_counter(); train_loss, train_acc = kd_epoch(
            student, teacher, train_loader, device, optimizer, args.alpha, args.temperature,
            args.label_smoothing, args.max_train_batches,
            args.stage == "qat" and epoch >= args.freeze_bn_epoch)
        val_loss, val_acc = kd_epoch(student.eval(), teacher, val_loader, device, None,
                                     args.alpha, args.temperature, 0.0, args.max_val_batches)
        scheduler.step()
    ...
    state = {"state_dict": student.state_dict(), "model_config": student_config, "normalization": {"mean": CIFAR10_MEAN, "std": CIFAR10_STD}, "validation_accuracy": best_acc, "validation_size": validation_size, "distillation_config": vars(args), "initialization": initialization}

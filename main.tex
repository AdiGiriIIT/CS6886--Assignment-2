...
Table~\ref{tab:baseline-results} reports the main baseline result. The best
checkpoint was selected by test top-1 accuracy; it occurred at epoch 19. The
last epoch achieved a nearly identical test accuracy of 92.66\%, showing that
the result is not due to a single unstable epoch.
...
\begin{table}[H]
    ...
    \begin{tabular}{lc}
        ...
        Best test top-1 accuracy & \textbf{92.69\%} \\
        Epoch of best checkpoint & 19 \\
        Final-epoch train accuracy & 97.06\% \\
        Final-epoch test accuracy & 92.66\% \\
        Best test loss & 0.2401 \\
        \bottomrule
    \end{tabular}
\end{table}
...
test accuracy increases from 62.07\% at epoch 2 to 84.59\% at epoch 3. It then
...
is 97.06\% while final test accuracy is 92.66\%, a gap of approximately 4.4
percentage points. Test loss reaches its minimum at epoch 19 and is essentially
flat thereafter, whereas training loss continues to decrease. Consequently,
